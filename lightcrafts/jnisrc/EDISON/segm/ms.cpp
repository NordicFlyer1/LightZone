...
void MeanShift::DefineKernel(float *h_, int *P_, int kp_)
{
	...
	//Allocate memory for h, P, offset, and increment
	...
	//Populate h and P, also use P to calculate
...
void MeanShift::MSVector(double *Mh_ptr, double *yk_ptr)
{
	...
	// Calculate search range for uniform kernel
	for(i = 0; i < kp; i++)
	{
		for(j = 0; j < P[i]; j++)
		{
	...
	// the search window (sphere) using uniform kernel
...
void MeanShift::LatticeMSVector(double *Mh_ptr, double *yk_ptr)
{
	...
	// window defined using uniform kernel
...
void MeanShift::generateLookupTable( void )
{
	...
	// lookup table w for uniform kernel

	for(i = 0; i < kp; i++)
	{
		// *Uniform Kernel* has weight function w(u) = 1
		// therefore, a weight function lookup table is
		// not needed for this kernel --> w[i] = NULL indicates this
